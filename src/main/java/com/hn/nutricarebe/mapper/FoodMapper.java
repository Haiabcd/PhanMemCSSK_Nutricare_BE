...
public interface FoodMapper {
    ...
    @Mapping(target = "ingredients",
            expression = "java(toRecipeIngredientResponses(food.getIngredients(), cdnHelper))")
    FoodResponse toFoodResponse(Food food, @Context CdnHelper cdnHelper);


    default Set<RecipeIngredientResponse> toRecipeIngredientResponses(Set<RecipeIngredient> set,
                                                                      @Context CdnHelper cdnHelper) {
        if (set == null || set.isEmpty()) return Collections.emptySet();

        return set.stream().map(ri -> {
            Ingredient ing = ri.getIngredient();
            String imageUrl = null;
            if (ing != null && ing.getImageKey() != null && !ing.getImageKey().isBlank()) {
                imageUrl = cdnHelper.buildUrl(ing.getImageKey());
            }
            return RecipeIngredientResponse.builder()
                    .ingredientId(ing != null ? ing.getId() : null)
                    .name(ing != null ? ing.getName() : null)
                    .unit(ing != null && ing.getUnit() != null ? ing.getUnit().name() : null)
                    .imageUrl(imageUrl)
